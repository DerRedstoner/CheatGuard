...
public final class Cuboid {
    ...
    public Cuboid(CustomLocation playerLocation) {
    ...
    public List<Block> getBlocks(World world) {
        ...
        if(!world.isChunkInUse((int) this.x1 >> 4, (int) this.z1 >> 4)) {
            return list;
        }
        if(!world.isChunkLoaded((int) this.x1 >> 4, (int) this.z1 >> 4)) {
            return list;
        }
    ...
    public List<Material> getBlockTypes(World world) throws IllegalStateException {
        ...
        try {
            ...
            list.add(world.getBlockAt(n, n3, n5).getType());

            for (int i = n; i < n2; ++i) {
                for (int j = n3; j < n4; ++j) {
                    for (int k = n5; k < n6; ++k) {
                        list.add(world.getBlockAt(i, j, k).getType());
                    }
                }
            }
        } catch(Exception e) {
